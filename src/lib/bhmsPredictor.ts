...
interface BhmsCollege {
  n: string;
  l: string;
  quotas: Record<string, Record<string, number[]>>; // quota -> category -> [R1] closing rank
}
...
// 2025 Round 1 only (the only round KEA had published an aggregated
// cutoff-rank report for at the time this was built).
export function predictBhms(rank: number, category: string, quota: QuotaType): BhmsResultRow[] {
