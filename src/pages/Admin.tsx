...
function BatchesPanel() {
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleCreate} className="bg-white rounded-3xl border-2 border-stone-200 p-6 space-y-4">
        <h3 className="font-bold text-lg text-stone-800">New Batch</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Batch name (e.g. PUC-II Science A)" className="border-2 border-stone-200 rounded-xl px-3 py-2" />
          <select value={examTrack} onChange={(e) => setExamTrack(e.target.value as Batch['examTrack'])} className="border-2 border-stone-200 rounded-xl px-3 py-2 bg-white">
            <option value="KCET">KCET</option>
            <option value="NEET">NEET</option>
            <option value="JEE">JEE</option>
            <option value="Other">Other</option>
          </select>
          <input type="number" value={year} onChange={(e) => setYear(parseInt(e.target.value, 10))} className="border-2 border-stone-200 rounded-xl px-3 py-2" />
        </div>
        <button type="submit" className="bg-amber-400 hover:bg-amber-300 text-stone-900 font-bold px-5 py-2 rounded-xl flex items-center gap-2">
      ...
      <div className="space-y-3">
        {batches.length === 0 && <p className="text-stone-400 italic text-sm">No batches yet — content without a batch is visible to all students.</p>}
        {batches.map((b) => (
          <div key={b.id} className="bg-white border-2 border-stone-200 rounded-2xl p-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Users className="w-4 h-4 text-stone-400" />
              <span className="font-bold text-stone-800">{b.name}</span>
              <span className="text-xs text-stone-500">{b.examTrack} · {b.year}</span>
            </div>
...
function BatchMultiSelect({ batches, selected, onChange }: { batches: Batch[]; selected: string[]; onChange: (ids: string[]) => void }) {
  ...
  if (batches.length === 0) return <p className="text-xs text-stone-400 italic">No batches created — this will be visible to all students.</p>;
  return (
      ...
      {batches.map((b) => (
        <button
          ...
          className={`text-xs font-bold px-3 py-1.5 rounded-full border-2 ${
            selected.includes(b.id) ? 'bg-stone-800 text-white border-stone-800' : 'border-stone-200 text-stone-500'
          }`}
        >
          {b.name}
        </button>
...
function ClassesAdminPanel() {
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleCreate} className="bg-white rounded-3xl border-2 border-stone-200 p-6 space-y-4">
        <h3 className="font-bold text-lg text-stone-800">Schedule a Live Class</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Class title" className="border-2 border-stone-200 rounded-xl px-3 py-2" />
          <input type="datetime-local" value={scheduledStart} onChange={(e) => setScheduledStart(e.target.value)} className="border-2 border-stone-200 rounded-xl px-3 py-2" />
          <input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Subject" className="border-2 border-stone-200 rounded-xl px-3 py-2" />
          <input value={chapter} onChange={(e) => setChapter(e.target.value)} placeholder="Chapter" className="border-2 border-stone-200 rounded-xl px-3 py-2" />
        </div>
        <input
          value={joinUrl}
        ...
        />
        <div>
          <label className="text-xs font-bold text-stone-500 uppercase tracking-widest mb-2 block">Target Batches</label>
      ...
      <div className="space-y-3">
        {classes.length === 0 && <p className="text-stone-400 italic text-sm">No classes scheduled yet.</p>}
        {classes.map((c) => {
          ...
          return (
            <div key={c.id} className="bg-white border-2 border-stone-200 rounded-2xl p-4">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full bg-stone-100 text-stone-600">{c.publishState}</span>
                ...
                <div className="flex items-center gap-1">
                  <select
                    ...
                    className="text-xs border-2 border-stone-200 rounded-lg px-2 py-1 bg-white"
                  >
...
function ContentAdminPanel() {
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleCreate} className="bg-white rounded-3xl border-2 border-stone-200 p-6 space-y-4">
        <h3 className="font-bold text-lg text-stone-800">Publish Notes / Video</h3>
        <div className="flex gap-2">
          {(['note', 'video'] as const).map((k) => (
            <button
              ...
              key={k}
              onClick={() => setKind(k)}
              className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider ${kind === k ? 'bg-stone-800 text-white' : 'bg-stone-100 text-stone-500'}`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Title" className="border-2 border-stone-200 rounded-xl px-3 py-2" />
          <input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Subject" className="border-2 border-stone-200 rounded-xl px-3 py-2" />
          <input value={chapter} onChange={(e) => setChapter(e.target.value)} placeholder="Chapter" className="border-2 border-stone-200 rounded-xl px-3 py-2" />
          <input
            value={url}
      ...
      <div className="space-y-3">
        {items.length === 0 && <p className="text-stone-400 italic text-sm">Nothing published yet.</p>}
        {items.map((item) => (
          <div key={item.id} className="bg-white border-2 border-stone-200 rounded-2xl p-4 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              {item.kind === 'note' ? <FileText className="w-5 h-5 text-stone-400" /> : <Video className="w-5 h-5 text-stone-400" />}
              <div>
...
function DoubtsInboxPanel() {
  ...
  if (threads.length === 0) return <p className="text-stone-400 italic text-sm">No doubts asked yet.</p>;
          ...
          <div key={t.id} className="bg-white border-2 border-stone-200 rounded-2xl p-5">
            <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-bold text-stone-800">{t.studentName} — {t.subject}</h4>
              </div>
              <button
                onClick={() => { setThreadStatus(t.id, t.status === 'resolved' ? 'open' : 'resolved'); forceUpdate(); }}
                className="text-xs font-bold uppercase tracking-wider text-stone-500 hover:text-stone-700"
              >
        ...
        {(
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-bold uppercase tracking-wider ${
              tab === key ? 'bg-stone-800 text-white' : 'bg-stone-100 text-stone-500'
            }`}
          >
            {label}
          </button>
